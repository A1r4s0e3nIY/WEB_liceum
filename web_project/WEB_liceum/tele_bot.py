...
async def plans(message: Message, state: FSMContext):
    await state.clear()
    await message.answer(f"Вы в меню планов", reply_markup=all_plans if plans_lst else plans_add_solo)
...
async def plan_add(message: Message, state: FSMContext):
    ...
    plans = await state.get_data()
    plans_lst.append(plans["plan"])
    await message.answer(f"План '{plans['plan']}' успешно добавлен")
    await message.answer(f"Вы в меню планов", reply_markup=all_plans)
    await state.clear()
...
async def plans_check(callback_query: CallbackQuery):
    await callback_query.answer()
    await callback_query.message.answer(f"Ваши планы на сегодня:\n{'\n'.join([str(i + 1) + '. ' + plans_lst[i] for i in range(len(plans_lst))])}", reply_markup=all_plans)
